...
exports.message = {
  preencode(state, m) {
    ...
    if (hasData) c.uint.preencode(state, m.data.byteLength)
  },
  encode(state, m) {
    ...
    if (hasData) c.uint.encode(state, m.data.byteLength)

    const end = state.start

    state.start = frame

    c.uint32.encode(state, end - start + (hasData ? m.data.byteLength : 0))
