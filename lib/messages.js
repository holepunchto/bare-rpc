...
const response = {
  ...
  decode (state) {
    ...
    if (error) return { error, stream, ...error.decode(state) }
    if (stream === 0) return { error, stream, data: c.buffer.decode(state) }

    return { error, stream, data: null }
  }
}
...
const stream = {
  ...
  decode (state) {
    ...
    if (stream & s.ERROR) return { stream, ...error.decode(state) }
    if (stream & s.DATA) return { stream, data: c.buffer.decode(state) }
