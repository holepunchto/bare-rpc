...
interface RPCIncomingRequest {
   ...
   * @param data - The payload to send: a `Buffer`, or a string encoded using `encoding`. Omit to
   * send no data.
  ...
  /**
   * Create a [`Writable`](https://github.com/mafintosh/streamx#writable-stream) stream for sending
   * the streamed reply to the request.
   * @param opts - Options for the returned
   * [`Writable`](https://github.com/mafintosh/streamx#writable-stream) stream.
   * @throws {ALREADY_SENT} a response has already been sent for this request.
   */
  createResponseStream(opts?: WritableOptions): RPCOutgoingStream
  /**
   * Create a [`Readable`](https://github.com/mafintosh/streamx#readable-stream) stream for
   * receiving the request's streamed data.
   * @param opts - Options for the returned
   * [`Readable`](https://github.com/mafintosh/streamx#readable-stream) stream.
...
interface RPCOutgoingRequest {
  ...
  /**
   * Await the reply from the remote end to the request. `encoding` can be defined for decoding the
   * response `data` buffer back into a string.
   * @param encoding - If given, decodes the reply payload to a string using this encoding; omit to
   * receive the raw `Buffer`.
   * @returns a promise that resolves with the remote end's reply payload, or rejects with the
   * channel's teardown error if it closes before a reply arrives.
  ...
  /**
   * Create a [`Readable`](https://github.com/mafintosh/streamx#readable-stream) stream for
   * receiving data in reply to the request.
   * @param opts - Options for the returned
   * [`Readable`](https://github.com/mafintosh/streamx#readable-stream) stream.
...
declare class RPCIncomingStream extends Readable {
   ...
   * @param type - Whether the stream carries the request body (`constants.type.REQUEST`) or the
   * response body (`constants.type.RESPONSE`).
   * @param opts - Options for the underlying
   * [`Readable`](https://github.com/mafintosh/streamx#readable-stream) stream.
   */
  constructor(
    rpc: RPC,
...
interface RPC {
  /**
   * Whether there are no requests or responses currently in flight. Useful for determining when it
   * is safe to tear down the underlying stream.
   */
  readonly idle: boolean

  /**
   * @param command - A unique number identifying the event; the remote end differentiates events by
   * it.
   * @returns an `RPCOutgoingEvent`; call `send()` on it to dispatch the one-way event.
   */
  event(command: number): RPCOutgoingEvent
  /**
   * @param command - A unique number identifying the request; the remote end differentiates
   * requests by it.
   * @returns an `RPCOutgoingRequest`; call `send()` on it to dispatch the request and `await
   * reply()` for the response.
   */
...
declare class RPC {
   ...
   * @param onrequest - Callback run for each incoming request or event, receiving an
   * `RPCIncomingRequest` (or `RPCIncomingEvent`) to inspect and `reply()` to. Defaults to a no-op.
   */
  constructor(stream: Duplex, onrequest: (req: RPCIncomingRequest) => void | Promise<void>)
}

/**
 * Create an RPC instance using a duplex `stream`. `onrequest` is an optional callback run when a
 * remote request is received. This is where processing and responding to an RPC request happens.
 * `onrequest` receives a `RPCIncomingRequest` as an argument.
 */
